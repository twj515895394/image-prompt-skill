...
当用户使用“角色设计图”“角色参考图”或“character sheet”等泛称，且未指定其他版式时，按 `references/tasks/character-assets/playbook.md` 中的默认“三大版面、四个功能区”执行：它是 **3 个主版面、4 个功能区**——正面无头全身、背面保留后脑但不露脸的全身、以及同一右侧大版面内上下排列的中性表情与微笑 3/4 面部特写。两张面部特写只改变表情，不改变身份、角度、光线、发型、服装和构图；正面全身的小脸必须移除，避免模型从远景小脸复制出错误身份。除非用户明确指定其他版式，不要退化为普通 2×2 网格，也不要把两个表情误解为两个不同角色。

只要角色是写实真人（用户明确要求写实 / 真人 / 照片质感，或参考图是真人照片），还必须读取 `references/controls/realism-quality/anti-ai-realism.md`，把皮肤五变量写入资产 Prompt。不要只写「真实皮肤」，也不要把四区板误判成原图保真增强。动漫、插画、像素等 stylized 角色不加载这页。
