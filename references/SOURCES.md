...
## 角色身份参考板版式修订（2026-08-19，2026-09-17 更新）
...
- 核心修订：默认 1920×1080、22% + 22% + 56%；只有第一列正面 Body Panel 去头，第二列必须保留完整后脑与背面全身但严格不露脸；右侧为单一正常表情头部 Close-up，只含头部、发型与发饰，不含颈部、肩线或服装。微笑、左转 45°、右转 45°等需求改为分别生成完整三分区变体。
...
- 2026-08-19：将角色身份参考板沉淀到 `character-assets`，并固定“仅第一列去头、第二列完整后脑”的默认规则。
- 2026-09-17：按用户确认将默认版式从四个功能区改为三个区；默认人脸区只保留正常表情，其他表情和脸部朝向改为独立三分区变体。
- 2026-09-22：按用户反馈补充泛称“角色图”的自动路由，并固定三分区默认使用纯灰白摄影棚背景，不继承参考图背景。
